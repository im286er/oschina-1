 ...
 */
public class HotTweet_LVAdapter extends BaseAdapter{
    List<Tweet_List> list = null ;

    public HotTweet_LVAdapter(List<Tweet_List>  list){
        this.list = list ;
    }
    ...
    public View getView(int position, View view,  ViewGroup parent) {
        ...
        holder.tv_content.setText(list.get(position).getBody()+"");
        ...
        if(!TextUtils.isEmpty(list.get(position).getImgSmall())){
            String imgUrl = list.get(position).getImgSmall();
            ...
            helper.httpRequest(imgUrl, new HttpHelper.BitmapCallBack() {
                ...
                public void getBack(Object bitmap) {
                    ...
                    new Handler(Looper.getMainLooper()).post(new Runnable() {
                        @Override
                        public void run() {
                            finalHolder.img_small.setVisibility(View.INVISIBLE);
